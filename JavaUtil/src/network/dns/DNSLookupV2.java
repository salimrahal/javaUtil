...
//            System.exit(-1);
//        }
        String domainTotest = "safirasoft.com";
        try {
            InetAddress inetAddress;
            // if first character is a digit then assume is an address
            if (Character.isDigit(domainTotest.charAt(0))) {   // convert address from string representation to byte array
                byte[] b = new byte[4];
                String[] bytes = domainTotest.split("[.]");
                ...
                inetAddress = InetAddress.getByName(domainTotest);
            }
            ...
            System.err.println("ERROR: No Internet Address for '" + domainTotest + "'");
        } catch (NamingException exception) {
            System.err.println("ERROR: No DNS record for '" + domainTotest + "'");
        }
    }
}
